...
export async function PATCH(
  ...
  try {
    ...
    const { rate_percentage, effective_date, expiry_date, is_active } = body;

    const rate = await SettingsService.updateLoanInterestRate(Number(id), {
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params;
    await SettingsService.deleteLoanInterestRate(Number(id));
    return NextResponse.json({ success: true });
  } catch (error: unknown) {
    console.error("Error deleting loan interest rate:", error);
